...
| Change the size list or the size chart | `SIZES`. `name` is what parents pick in the size dropdown and what goes on the PayPal receipt and in the Sheet. `waist` and `height` appear only in the **Size chart** popup (waist values come from the SanMar size chart SizeChart-PA-V3). Use curly quotes `”` `’` for inches and feet; a straight `"` breaks the store. |
